...
const RequestView: React.FC = () => {
  ...
  return (
    <div className="p-4 lg:p-8 h-full overflow-y-auto bg-brandDark pb-32">
      <div className="max-w-6xl mx-auto glass-panel rounded-[40px] p-6 lg:p-10 backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          <div>
            <h2 className="text-2xl md:text-3xl font-black italic text-white uppercase tracking-tighter">Görev & İstek Yönetimi</h2>
            <p className="text-gray-500 text-[10px] mt-1 uppercase tracking-widest font-black">Asistan tarafından oluşturulan tüm direktifler</p>
          </div>
          <div className="flex flex-wrap gap-4 w-full md:w-auto">
            <button
              onClick={clearDone}
              className="flex-1 md:flex-none px-6 py-4 bg-white/5 hover:bg-white/10 text-gray-400 font-black uppercase tracking-widest rounded-2xl transition-all border border-white/10 text-[10px]"
            >
              Temizle
            </button>
            <button
              ...
              className="flex-1 md:flex-none px-8 py-4 bg-primary hover:brightness-110 text-white font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-primary/20 text-[10px]"
            >
              Manuel Ekle
            </button>
          </div>
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="border-b border-white/5 text-[9px] text-gray-500 uppercase font-black tracking-[0.2em]">
                <th className="px-6 py-4 text-primary italic">ID</th>
            ...
            <tbody className="text-[10px] font-bold uppercase tracking-widest">
              {requests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-gray-600 italic uppercase tracking-[0.3em]">Görev listesi boş.</td>
                </tr>
              ) : requests.map((r) => (
                <tr key={r.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-6 text-primary italic font-mono">{r.id}</td>
                  <td className={`px-6 py-6 ${r.status === 'success' ? 'text-gray-600 line-through opacity-50' : 'text-gray-200'}`}>
                    {r.topic}
                  </td>
                  ...
                  <td className="px-6 py-6 text-gray-500 font-mono">{r.date}</td>
                  <td className="px-6 py-6">
                    <button
