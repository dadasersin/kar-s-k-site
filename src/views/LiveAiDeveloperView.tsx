...
const LiveAiDeveloperView: React.FC = () => {
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      addLog('Portal modülleri arası senkronizasyon katmanı hazırlanıyor...');
      ...
      const model = genAI.getGenerativeModel({ model: keys[0].modelName || "gemini-1.5-flash-latest" });

      const aiPrompt = `
        Sen bir React ve Tailwind CSS uzmanısın ve "Ersin Güleş Portal" sisteminin baş mimarısın.

        GÖREV: Kullanıcının "${prompt}" isteğini yerine getiren SENKRONİZE bir modül üret.

        DİKKAT:
        1. Bu modül portalın diğer özellikleri ile senkronize çalışmalı.
        2. Eğer kullanıcı veri istiyorsa (borsa, kripto, log), localStorage üzerinden ilgili anahtarları (bist_favorites, crypto_watchlist, chat_history vb.) okumaya çalış.
        3. Bilgi Bankasından gelen verileri (aşağıda) baz alarak gerçek çalışan mantık kur.
        4. Modern, koyu tema (glassmorphism) ve neon mavi vurgular kullan.

        TEKNİK BİLGİ BANKASI VE KAYITLAR:
        ${knowledge}

        CIKTI FORMATI:
        Lütfen sadece tek bir HTML stringi ver. Tailwind sınıfları kullan.
        İçinde <script> etiketleri ile logicleri (localStorage okuma/yazma, fetch, filter vb.) barındırsın.
        Markdown blockları (\`\`\`) kullanma, direkt kodu ver.
      `;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-sm font-bold tracking-widest uppercase">Otonom Senkronizasyon ve Portal Mimarı</p>
          </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="glass-panel p-6 rounded-[2rem] border border-white/5 bg-surface/30 shadow-2xl">
               ...
               <textarea
                 ...
                 placeholder="Siteye ne eklemek istersiniz? Diğer tüm modüllerle senkronize olup otonom olarak inşa edilecektir..."
